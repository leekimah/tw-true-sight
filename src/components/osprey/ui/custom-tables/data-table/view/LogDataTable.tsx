...
import { HiArrowPath, HiArrowPathRoundedSquare, HiPrinter } from 'react-icons/hi2';
...
export const LogDataTable = <T extends unknown>({
  datasource,
  columns,
  queryKey,
  onRowClick,
  ...
  return (
    <div className={`${className}`}>
      ...
      <div className="flex w-full justify-end px-4 bg-white pb-4 gap-2">
        <button
          onClick={() => table?.resetColumnFilters()}
          className="bg-orange-500 px-2 rounded font-semibold text-sm text-white w-[6rem]"
        >
          Reset
        </button>
        <button
          className="px-3 py-1 text-lg bg-slate-600 rounded text-white flex gap-2 items-center"
          onClick={() => console.log(table?._getFilteredRowModel!().rows.map((row) => row.original))}
        >
          <HiPrinter /> Print
        </button>
      </div>

      {/* <div className="w-full flex justify-end gap-2 bg-white py-2 px-2">
        <div>
          <span className="text-xs">From</span>{' '}
          <input type="date" className="bg-slate-200 px-2 py-1 rounded text-sm text-gray-600" />
        </div>
        <div>
        ...
        <button className="px-3 bg-orange-600 rounded text-white text-xs" onClick={() => table?.resetColumnFilters()}>
          Reset
        </button>
